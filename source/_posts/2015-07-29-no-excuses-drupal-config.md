...
**NOTE** This post is a continuation of [No Excuses Part I](http://craychee.io/blog/2015/05/20/no-excuse-config-management-drupal/). This post picks up right where that left off so read and follow that first.

SSH into your newly built machine by running `vagrant ssh`. Then navigate to `/vagrant/www` and run `drush si --db-url=mysql://default:default@localhost/default y` (or whatever you set the user/password/database config to).
...
###Step 1. Get your settings.
One of the benefits of working a known state is that we know how to connect to the database. We can actually share a local `settings.php` without anyone else needing to think about it.
...
Remember that we ultimately want a site install to happen when we run out script. We started by manually running `drush si --db-url=mysql://default:default@localhost/default y`. We created a settings file but we need to get it into the `sites/default/` directory of our Drupal and then pipe drush commands to the project.
...
Now one problem that isn't a problem yet but will be is that the `settings.php` that we are symlinking into Drupal on our build will be different based on the environment. We want the same build script to run (`install.sh`) no matter what.

There are a number of ways to sort handle environment specific config. This is how we are going to do it:
